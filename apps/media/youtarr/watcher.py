...
# Youtarr writes a Kodi/Jellyfin-style .nfo alongside every video (nfoGenerator.js).
NFO_EXTS = {".nfo"}
# Youtarr also writes a per-channel .m3u playlist that isn't tied to any single
# video's stem — never bundled with a move, only ever cleaned up as an orphan
# once every video has been relocated out of its channel folder.
PLAYLIST_EXTS = {".m3u"}
...
ORPHAN_SUFFIXES = {".info.json"} | THUMB_EXTS | SUB_EXTS | NFO_EXTS | PLAYLIST_EXTS
...
def get_openai():
    ...
    _openai_client = OpenAI(api_key=OPENAI_API_KEY)  # security-agent: ignore
    return _openai_client
...
def bundle_paths(media_path: Path) -> List[Path]:
    ...
    We handle:
      ...
      - Foo.nfo or Foo.mp4.nfo (Youtarr)
    """
    bundle = [media_path]
    ...
    # thumbs, subs, and Youtarr's .nfo can be either Foo.webp OR Foo.mp4.webp
    for ext in THUMB_EXTS | SUB_EXTS | NFO_EXTS:
